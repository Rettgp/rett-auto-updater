...
_addon.name = 'bb-auto-updater'
_addon.version = '1.0.0'
_addon.author = 'Rettgp'
_addon.commands = {'bbautoupdater', 'bbau'}
...
local function update_addon(addon_name, download_url)
   ...
   notice(207, ('[bb-auto-updater] Would update %s from %s'):format(addon_name, download_url))
end

windower.register_event('load', function()
    windower.add_to_chat(207, '[bb-auto-updater] Checking for addon updates...')
    local manifest_data = {}
    local response = {}
    ...
    if code ~= 200 then
        error(123, '[bb-auto-updater] Failed to fetch manifest: HTTP ' .. tostring(code))
        return
    end
    ...
    if not success or not manifest then
        error(123, '[bb-auto-updater] Failed to parse manifest.json')
        return
    end
    for addon_name, remote_info in pairs(manifest.addons or {}) do
        ...
        if local_version and local_version ~= remote_info.version then
            windower.add_to_chat(207, ('[bb-auto-updater] %s is outdated (local: %s, remote: %s)'):format(addon_name, local_version, remote_info.version))
            update_addon(addon_name, remote_info.download_url or ADDONS_BASE_URL)
        end
    end
    notice(207, '[bb-auto-updater] Update check complete.')
end)

-- Command: Manual update check
windower.register_event('addon command', function(cmd)
    if cmd == 'check' then
        windower.send_command('lua reload bb-auto-updater')
    end
end)
